...
function getUserInfo(i) {
	con.query('SELECT Id FROM users WHERE Fornavn ="' + i + '"', (err, rows) => { //Selects only users with first name
		if (err) throw err;
		con.query('SELECT * FROM users', (err, student) => { // makes it possible to get every user and select from them
			if (err) throw err;
...
function getNewsTitle() {
	var news = []
	con.query("SELECT * FROM news ORDER BY Startdate ASC", (err, rows) => {
		if (err) throw err;
		for (var i = 0; i < rows.length; i++) {
...
function getNewsStartDate() {
	var news = []
	con.query("SELECT * FROM news ORDER BY Startdate ASC", (err, rows) => {
		if (err) console.log(err);
		for (var i = 0; i < rows.length; i++) {
			news.push(rows[i].Startdate + ":-:")
		}
	})
	return news
}

function getNewsEndDate() {
	var news = []
	con.query("SELECT * FROM news ORDER BY Startdate ASC", (err, rows) => {
		if (err) console.log(err);
		for (var i = 0; i < rows.length; i++) {
			news.push(rows[i].Enddate + ":-:")
		}
	})
	return news
}
...
function removeNews() {
	con.query("DELETE FROM news WHERE Enddate < now() - interval 1 DAY;", (err, rows) => {
